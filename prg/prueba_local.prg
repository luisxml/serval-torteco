...
PUBLIC Vgn_bascula,Vgc_CERTIFICACION_INEA, VGC_VERSION,Vcl_conex,VPC_llave,Vgc_caja,Vgc_impresora,Vgc_impresora2,Vgc_barras, Vgc_direccion 
Vgc_barras =0  
*SET STEP ON
...
lsql="Select DIRECCION FROM SUCURSAL WHERE FG_ACTIVO = 1 AND ID_SUCURSAL = ?Vgc_almatrab"
resp=SQLEXEC(conex, lsql, "DIRECCION")
IF resp<0
	MESSAGEBOX("Disculpe, error en la consulta, por favor comunicarse con el personal de soporte tecnico.",0+16,"Error de conexión")
	RETURN 
ELSE
	SELECT DIRECCION
	Vgc_direccion = ALLTRIM(DIRECCION)	
	USE IN DIRECCION
ENDIF 
